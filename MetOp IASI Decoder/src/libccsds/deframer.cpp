...
namespace libccsds
{
    CADUDeframer::CADUDeframer(bool derandomize)
    {
        if (derand_m)
        {
    ...
    std::vector<CADU> CADUDeframer::work(uint8_t *input, size_t size)
    {
        // Output buffer
        std::vector<CADU> frames;
        ...
        for (int byteInBuf = 0; byteInBuf < size; byteInBuf++)
        {
            ...
            for (int i = 7; i >= 0; i--)
            {
                ...
                if (writeFrame)
                {
                    ...
                    if (--wroteBits == 0)
                    {
                        frameBuffer[wroteBytes] = derand_m ? outBuffer ^ d_rantab[wroteBytes] : outBuffer;
                        wroteBytes++;
                        wroteBits = 8;
                    }
                ...
                // errors (here's why errors get reset each time a frame is good) means reset to state 0
